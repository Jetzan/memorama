...
export const loginOrRegister = async (
  ...
  try {
    ...
    if (rows.length > 0) {
      ...
      await pool.query(
        "INSERT INTO usuarios (nombre, tiempoFacil, tiempoMedio, tiempoDificil, puntajeFacil, puntajeMedio, puntajeDificil) VALUES (?, ?, ?, ?, ?, ?, ?)",
        [nombre, 0, 0, 0, 0, 0, 0]
      );
      ...
      const jugador = new Jugador(
        ...
        rows[0].tiempoFacil,
        rows[0].tiempoMedio,
        rows[0].tiempoDificil,
        rows[0].puntajeFacil,
        rows[0].puntajeMedio,
        rows[0].puntajeDificil
      );
      return res
        .status(201)
        .json({ mensaje: "Usuario registrado", usuario: nuevo[0] });
    }
  } catch (error) {
...
export const actualizarJugador = async (req: Request, res: Response) => {
  ...
  if (!nombre || !campos || typeof campos !== "object")
    return res
      .status(400)
      .json({ error: "Datos incompletos o formato inválido" });
  ...
  const setPartes = Object.keys(campos)
    .map((campo) => `${campo} = ?`)
    .join(", ");
